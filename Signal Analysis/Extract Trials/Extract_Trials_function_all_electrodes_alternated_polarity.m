function Extract_Trials_function_all_electrodes_alternated_polarity(Files_Mat_file_selected,Files_Mat_file_directory,Seconds_Before_trigger,...
    ...
    artifact_pos_electrode,adj_trigger_value,max_N_sweeps,eeg_meg_scalp_map,...
     ...
     check_baseline_correction, time_baseline_correction)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Combining the two triggers together 
temp_trig_all = zeros(1,length(triggers_analysis)*2);

temp_trig_all(1:2:end) = triggers_analysis(1,:);
temp_trig_all(2:2:end) = triggers_analysis(2,:);

triggers_analysis = [];
triggers_analysis = temp_trig_all;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%Finding the starting time for the baseline correction
if (check_baseline_correction == 1)
    
    samples_baseline_correction = find(tt_mean >= time_baseline_correction,1,'First');
...
display(['Sweep# ' num2str(kk)]);   
    ...
    if (min(min(temp_eeg_rejection)) > artifact_neg_electrode && max(max(temp_eeg_rejection)) < artifact_pos_electrode)
         ...
         %Check if baseline correction needs to be applied
                if (check_baseline_correction == 1)
                   
                    temp_eeg_rejection = temp_eeg_rejection - mean(temp_eeg_rejection(:,samples_baseline_correction:find_onset),2);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Plotting the saving all the channels
if (plot_save_av_chan == 1)
...
for ll = 1:size(save_single_trials,1)
    ...
    if(size(save_single_trials,2) > 1)
        ...
        if(standardized_data == 1)
            ...
            plot(1000*tt_mean,zscore(mean(squeeze(save_single_trials(ll,:,:)))))
    ...
    plot(1000*tt_mean,mean(squeeze(save_single_trials(ll,:,:))))
       ...
       if(standardized_data == 1)
           ...
           plot(1000*tt_mean,zscore(squeeze(save_single_trials(ll,:,:))))
    ...
    if(size(save_single_trials,2) > 1)
        ...
        if(standardized_data == 1)
             ...
             plot([tt_mean(find_onset) tt_mean(find_onset)],[min(zscore(mean(squeeze(save_single_trials(ll,:,:))))) max(zscore(mean(squeeze(save_single_trials(ll,:,:)))))],'Color','k','LineWidth',1); 
     ...
     plot([tt_mean(find_onset) tt_mean(find_onset)],[min(mean(squeeze(save_single_trials(ll,:,:)))) max(mean(squeeze(save_single_trials(ll,:,:))))],'Color','k','LineWidth',1); 
        ...
        if(standardized_data == 1)
            ...
            plot([tt_mean(find_onset) tt_mean(find_onset)],[min(zscore(squeeze(save_single_trials(ll,:,:)))) max(zscore(squeeze(save_single_trials(ll,:,:))))],'Color','k','LineWidth',1);  
...
data_exported.alternated_polarity = 'Yes';
...
extract_peaks_RMS_Biosemi(data_exported.average_trials,tt_mean,channels,standardized_data,name_file_saved,data_eeg.data_exported.sampling_frequency,...
    ...
    extract_peaks_RMS_MEG(squeeze(mean(save_single_trials,2)),tt_mean,channels,standardized_data,name_file_saved,data_eeg.data_exported.sampling_frequency,...
        P1_start_window,P1_end_window,N1_start_window,N1_end_window,P2_start_window,P2_end_window,Start_RMS,End_RMS,abs_val_peaks)
...
try    
%% Creating the scalpmap for up to 3 peaks selected by the users. Iy usually is P1 (M50), N1 (M100) and P2 (M200)
...
if (eeg_meg_scalp_map == 1)
    ...
    scalp_map_MEG(data_exported.sensors_removed,data_exported.average_trials,tt_mean,channels,standardized_data,name_file_saved,data_eeg.data_exported.sampling_frequency,...
        P1_start_window,P1_end_window,N1_start_window,N1_end_window,P2_start_window,P2_end_window,Start_RMS,End_RMS,channels_names,data_eeg.data_exported.labels,abs_val_peaks)
