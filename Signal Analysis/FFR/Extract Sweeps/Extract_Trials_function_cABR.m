...
%time_domain = [-1:size(eeg_sweeps,2) - 2]/sampling_frequency + adj_trigger_value - 0.9/1000 + Seconds_Before_trigger;
 ...
 time_domain = [0:size(temp_eeg_rar,2) - 1]/sampling_frequency + round(adj_trigger_value*sampling_frequency)/sampling_frequency + round(Seconds_Before_trigger*sampling_frequency)/sampling_frequency;

else
   
 %time_domain = [0:size(eeg_sweeps,2) - 1]/sampling_frequency + Seconds_Before_trigger; 
 time_domain = [0:size(temp_eeg_rar,2) - 1]/sampling_frequency + round(Seconds_Before_trigger*sampling_frequency)/sampling_frequency;
...
if (check_baseline_correction == 1)
    ...
    samples_baseline_correction = find(time_domain >= time_baseline_correction,1,'First');
  ...
  if (min(temp_eeg_rar) > artifact_neg_electrode && max(temp_eeg_rar) < artifact_pos_electrode && sweeps_rar < max_trials)
      ...
      if (check_baseline_correction == 1)
          ...
          temp_eeg_rar = temp_eeg_rar - mean(temp_eeg_rar(:,samples_baseline_correction:find_onset),2);
...
%% This part of the code will be executed only if two different triggers were detected   
    ...
    if (min(temp_eeg_compr) > artifact_neg_electrode && max(temp_eeg_compr) < artifact_pos_electrode && sweeps_compr < max_trials)
         ...
         if (check_baseline_correction == 1)
            ...
            temp_eeg_compr = temp_eeg_compr - mean(temp_eeg_compr(:,samples_baseline_correction:find_onset),2);
