function DSS_EEG_Concatenate(DSS_EEG_file_directory,start_p1_time,end_p1_time,start_N100_time,end_N100_time,start_P200_time,end_P200_time, ...
...
for ii = 3:files_number
  ...
  if (strcmp(matrix_file(1,end-2:end),'mat') == 1)
      ...
      if (filter_DSS_var == 1)
          ...
          data_filtered = zeros(size(selected_file.data_exported.single_trials,1),size(selected_file.data_exported.single_trials,2),size(selected_file.data_exported.single_trials,3));
          
          for kk = 1:size(selected_file.data_exported.single_trials,1)
              
              for ll = 1:size(selected_file.data_exported.single_trials,2)
                  
                  if (phase_filter == 1)
                      
                      data_filtered(kk,ll,:) = filtfilt(bb_filter,aa_filter,squeeze(selected_file.data_exported.single_trials(kk,ll,:)));
                      
                  else
                      
                      data_filtered(kk,ll,:) = filter(bb_filter,aa_filter,squeeze(selected_file.data_exported.single_trials(kk,ll,:)));
          ...
          single_trials_clean = selected_file.data_exported.single_trials;
          selected_file.data_exported.single_trials = data_filtered;
      ...
      try
          ...
          if track_subjects == 1
              ...
              temp_data_clean = zeros(size(selected_file.data_exported.single_trials,1),size(selected_file.data_exported.single_trials,2),size(selected_file.data_exported.single_trials,3));
          ...
          for tt = 1:size(selected_file.data_exported.single_trials,2)
              ...
              temp_data_clean(:,tt,temp_size_clean + 1:size(selected_file.data_exported.single_trials(:,tt,:),3) + temp_size_clean) = single_trials_clean(:,tt,:); 
...
if sweeps_analysis_DSS < size(selected_file.data_exported.single_trials,2)
    ...
    temp_data_clean = temp_data_clean(:,1:sweeps_analysis_DSS,:);
...
for hh = 1:size(temp_data,2)
    ...
    temp_data_rearrenge_clean = squeeze(temp_data_clean(:,hh,:))';
    
    %data_dss(:,:,hh) = filter(bb,aa,temp_data_rearrenge);
    data_dss(:,:,hh) = temp_data_rearrenge;
    clean(:,:,hh) = temp_data_rearrenge_clean;
