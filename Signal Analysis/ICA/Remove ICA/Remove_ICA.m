...
try
    ...
    temp_ica = get(handles.ICA_to_be_Removed,'String');
    
    for ll = 1:length(temp_ica)

ica_to_remove(ll,1) = str2num(cell2mat(temp_ica(ll)));
...
%ica_selected = struct_data.data_exported.eeg_data(ica_to_remove,:);
%projected_ica = [];
projected_ica = zeros(size(struct_data_ICA.data_exported.eeg_data,1),size(struct_data_ICA.data_exported.eeg_data,2));
for kk = 1:length(ica_to_remove)

projected_ica = projected_ica + struct_data_ICA.data_exported.A(:,ica_to_remove(kk)).*struct_data_ICA.data_exported.eeg_data(ica_to_remove(kk),:); %Project out the data in the sensor space; 
...
data_exported.ICAs = ica_to_remove;
