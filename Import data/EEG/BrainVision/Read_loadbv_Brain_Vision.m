...
% $Id: pop_loadbv.m 32 2007-04-02 17:02:49Z andreaswidmann $

%% Changes to the original "EEGLab" code
%function [EEG, com] = Read_loadbv_Brain_Vision(path, hdrfile, srange, chans);

function [EEG, com] = Read_loadbv_Brain_Vision(path, hdrfile);
...
%%"hdrfile" and "path" have already been selected;
...
%% Change to the original "EEGLab" code
              %%The function "inputgui" has been removed, as it is assumed that all
    %%the samples and channeles will be save
        %result = inputgui(uigeom, uilist, 'pophelp(''pop_loadbv'')', 'Load a Brain Vision Data Exchange format dataset');
        
        %Default values will be used => all the electrodes and all the samples;
        result = [{''} {''}];
