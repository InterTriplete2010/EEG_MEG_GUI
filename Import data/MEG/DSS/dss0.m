...
c3=N'*topcs1'*c2*topcs1*N; %Rotating c2 into a new base created by the whitened rotation matrix calculated in "pcarot(c1)"; 
%In order to white an orthogonal basis (e.g. eigenvectors of PCA), you
%simply multiply the eingenvectors by the root of the eingenvalues (see https://theclevermachine.wordpress.com/tag/eigenvalue-decomposition/)
%See "Matrix similarity" to the property of similar matrices; c3 and c2 are similar matrices, 
% which means det(c3 - lambda*I) =  det (N'*topcs1'*c2*topcs1*N - lambda*I);

% derive the dss matrix
[topcs2,evs2]=pcarot(c3);
todss=(topcs1*N)*topcs2; %topics1*N are the whitened eingenvectors (or rotation matrix) extracted from the first PCA;
fromdss=pinv(todss);    %The pseudo-inverse matrix is calculated by using the Singular Value Decomposition (SVD) approach;
%Given M = USV', then the pseudo-inverse matrix of M is: VSU', where S is formed by calculating the reciprocal of any non-zero
%values in the diagonal and by transposing the resulting matrix
