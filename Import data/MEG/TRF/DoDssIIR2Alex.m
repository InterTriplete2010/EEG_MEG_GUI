function DoDssIIR2Alex(Forder,freqL,freqH,order_DSS1,Freq_Low_DSS1,Freq_High_DSS1,wav_file_directory, ...
    wav_file_selected,mat_file_directory,n_components,wav_file_selected_noise,wav_file_directory_noise,SF_MEG_Data, time_shift_DSS,Subjcet_Number_analysis,decimation_fact,zero_phase_menu,Filter_DSS,Filter_Auditory_Stim)
...
%[b_env,a_env] = butter(order_DSS1,Freq_High_DSS1/(Fs/2),'Low'); 
...
save_std = [];
save_corr = [];
save_all_corr_cross_val = zeros(Subjcet_Number_analysis,10);
%save_optimal_filter = zeros(n_components,shift_samples,Subjcet_Number_analysis);
...
save_median_noise = [];
save_std_noise = [];
save_all_corr_cross_val_noise = zeros(Subjcet_Number_analysis,10);
...
%Decoding MEG data
[temp_h,CR_test_out,Str_testE_best,Str_TrainE,Best_iter,Total_Iter,save_max,measured_env_best,pred_env_best] = svdboostV4_Alex(x_res_analysis,y_res_analysis(1,1:size(x_res_analysis,2)),cross_folders-1,...
...
[save_max_optimal_filter] = optimal_filter_background_noise(x_res_analysis,y_res_analysis_noise(1,1:size(x_res_analysis,2)),h,cross_folders - 1);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Saving the values for the target stimulus
save_all_corr_cross_val(track_subjects,:) = save_max(2,:);
...
save_optimal_filter(:,:,track_subjects) = h;
...
save_all_corr_cross_val_noise(track_subjects,:) = save_max_optimal_filter(2,:);
...
save_mean_data.measured_speech_env = measured_env_best_matrix;
save_mean_data.predicted_speech_env = pred_env_best_matrix;

create_dir = 'Backward_Av_Save_Corr';
...
save_corr_target = ['Backward_Av_Corr_Target_' wav_file_selected(1,1:end-4) '_' num2str(time_shift_DSS(1,integration_windows)) '.mat'];
...
save_corr_noise = ['Backward_Av_Corr_Noise_' wav_file_selected_noise(1,1:end-4) '_' num2str(time_shift_DSS(1,integration_windows)) '.mat'];
